...
export default function EleveChatAdmin() {
  ...
  // 📜 Scroll auto vers le dernier message — seulement à l'ouverture ou nouveau message
  useEffect(() => {
  ...
  // ✉️ Envoyer un message (texte, fichier, image, vidéo ou vocal)
  const handleSend = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!messageText.trim() && selectedFiles.length === 0 && !audioBlob) return
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 140px)', background: '#fff', borderRadius: 8, border: '1px solid #e5e7eb', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '10px 16px', background: '#f0f2f5', borderBottom: '1px solid rgba(0,0,0,.06)', display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <div>
          <p style={{ margin: 0, fontSize: 14, fontWeight: 600, color: '#111' }}>Administration</p>
          <p style={{ margin: 0, fontSize: 11, color: '#667781' }}>Échangez directement avec l'équipe administrative</p>
        </div>
      </div>
      ...
      <div style={{
        ...
        {messages.length === 0 ? (
          <div style={{ textAlign: 'center', margin: 'auto', color: '#94a3b8' }}>
            ...
            <p style={{ fontSize: 13 }}>Aucun message. Commencez la conversation !</p>
          </div>
        ) : (
              ...
              <Fragment key={msg.id}>
                ...
                >
                  <div style={{
                    ...
                    background: isMe ? 'linear-gradient(135deg, #dcf8c6 0%, #c8e6c9 100%)' : '#ffffff',
                    boxShadow: '0 2px 8px rgba(0,0,0,0.06)',
                    overflow: 'hidden',
                    border: isMe ? '1px solid #b2dfdb' : '1px solid #e0e0e0',
                  }}>
                    {/* En-tête (nom de l'expéditeur si ce n'est pas l'élève) */}
                    {!isMe && (
                      <div style={{ padding: '10px 14px 4px' }}>
                        <span style={{ fontSize: '12px', fontWeight: 700, color: '#5c3317' }}>
                          {msg.expediteur_nom || 'Administration'}
                        </span>
                      </div>
                    )}
                      ...
                      >
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontWeight: 700, color: isMe ? '#2e7d32' : '#25d366', marginBottom: 2 }}>
                            {repliedMsg.expediteur_nom === user?.display_name ? 'Vous' : (repliedMsg.expediteur_nom || 'Administration')}
                          </div>
                    ...
                    {msg.type_message === 'image' && fileUrl && (
                      <div style={{ position: 'relative', background: '#f0f0f0' }}>
                        <img
                          ...
                          onClick={() => setPreviewMedia({ url: fileUrl, type: 'image', name: msg.nom_fichier || undefined })}
                          onError={(e) => { (e.target as HTMLImageElement).style.display = 'none' }}
                        />
                        {warning && (
                    ...
                    {msg.type_message === 'video' && fileUrl && (
                      <div style={{ position: 'relative', background: '#000', overflow: 'hidden' }}>
                        <video
                          ...
                          onClick={() => setPreviewMedia({ url: fileUrl, type: 'video', name: msg.nom_fichier || undefined })}
                        />
                        {warning && (
                          <span style={{ position: 'absolute', bottom: 8, right: 8, background: 'rgba(220, 38, 38, 0.9)', color: 'white', fontSize: '10px', padding: '4px 8px', borderRadius: '12px', fontWeight: 600, backdropFilter: 'blur(4px)' }}>
                            {warning}
                    ...
                    {msg.type_message === 'audio' && fileUrl && (
                      <div style={{ padding: '8px 14px 12px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', background: isMe ? 'rgba(255,255,255,0.6)' : '#f5f5f5', padding: '8px 12px', borderRadius: '24px' }}>
                          ...
                          <audio controls src={fileUrl} style={{ height: '32px', maxWidth: '200px', outline: 'none' }} />
                        </div>
      ...
      <form onSubmit={handleSend} style={{ padding: '10px 16px', background: '#f0f2f5', display: 'flex', flexDirection: 'column', gap: 8, borderTop: '1px solid rgba(0,0,0,.06)', position: 'relative' }}>
          ...
          <div style={{
            ...
            <div style={{ flex: 1, overflow: 'hidden' }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: '#25d366', marginBottom: 2 }}>
                Répondre à {replyToMessage.expediteur_nom === user?.display_name ? 'vous' : (replyToMessage.expediteur_nom || 'Administration')}
              </div>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          <input type="text" value={messageText} onChange={e => setMessageText(e.target.value)} placeholder="Écrivez votre message..." style={{ flex: 1, padding: '10px 16px', borderRadius: 24, border: 'none', background: '#fff', fontSize: 15, outline: 'none', boxShadow: '0 1px 3px rgba(0,0,0,.05)' }} maxLength={1000} />
