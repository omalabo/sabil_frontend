...
export default function DirectionChat() {
  ...
  // 📡 Fetch des élèves (uniquement ceux gérés par cet admin)
    const { data: usersData } = useGetUsersQuery({ role: 'admin', page: 1 }, { skip: user?.role !== 'direction' })
  ...
  const handleSend = async (e: React.FormEvent) => {
    e.preventDefault()
    if ((!messageText.trim() && selectedFiles.length === 0 && !audioBlob) || !selectedAdmin) return

    try {
      const formData = new FormData()
      formData.append('destinataire', selectedAdmin.id)
    ...
    <div style={{ margin: -24, display: 'flex', height: '92vh', overflow: 'hidden', background: '#f0f2f5' }}>
      ...
      <div style={{
        ...
        <div className="p-4 border-b border-neutral-200">
          ...
          <p className="text-xs text-neutral-500">Direction ↔ admin uniquement</p>
        </div>

        <div className="flex-1 overflow-y-auto">
          {admins.map((eleve: User) => (
            <button
              key={eleve.id}
              onClick={() => {setSelectedAdmin(eleve); if (window.innerWidth < 768) setLeftPanelOpen(false)}}
              className={`w-full text-left p-3 border-b border-neutral-100 hover:bg-neutral-50 transition-colors ${
                selectedAdmin?.id === eleve.id ? 'bg-primary-50 border-l-2 border-primary-500' : ''
              }`}
            >
              <p className="font-medium text-sm text-neutral-900">{eleve.display_name || eleve.email}</p>
              <p className="text-xs text-neutral-500 truncate">{eleve.email}</p>
            </button>
          ))}
          {admins.length === 0 && (
            <p className="p-4 text-sm text-neutral-500 text-center">Aucun admin à contacter</p>
          )}
        </div>
      </div>
      ...
      <div style={{ 
        flex: 1, display: (!selectedAdmin && window.innerWidth < 768) ? 'none' : 'flex', 
        ...
        {selectedAdmin ? (
            ...
            {selectedMsgIds.size > 0 ? (
              ...
              <div style={{ padding: '10px 16px', background: '#f0f2f5', borderBottom: '1px solid rgba(0,0,0,.06)', display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'linear-gradient(135deg, #312e81, #1e1b4b)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, color: '#fff', fontWeight: 700 }}>
                  {selectedAdmin.display_name?.[0]?.toUpperCase() || '🏫'}
                </div>
                <div>
                  <p style={{ margin: 0, fontSize: 14, fontWeight: 600, color: '#111' }}>{selectedAdmin.display_name}</p>
                  <p style={{ margin: 0, fontSize: 11, color: '#667781' }}>{selectedAdmin.email}</p>
                </div>
              </div>
            )}
            ...
            <div style={{
              ...
              {loading ? (
                          ...
                          {!isMe && (
                            <div style={{ padding: '10px 14px 4px' }}>
                              <span style={{ fontSize: '12px', fontWeight: 700, color: '#5c3317' }}>
                                {msg.expediteur_nom || selectedAdmin.display_name}
                              </span>
                            </div>
                          )}
                ...
                <div style={{ textAlign: 'center', margin: 'auto', color: '#94a3b8' }}>
                  ...
                  <p style={{ fontSize: 13 }}>Aucun message avec {selectedAdmin.display_name}. Commencez la conversation !</p>
                </div>
              )}
          ...
          <div className="flex-1 flex items-center justify-center text-neutral-400">
            <div className="text-center">
              <p>👈 Sélectionnez un admin à gauche pour commencer une conversation privée</p>
            </div>
          </div>
        ) : null}
      </div>
