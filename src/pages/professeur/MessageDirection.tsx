...
export default function ProfMessageDirection() {
  ...
  useEffect(() => {
    api.get('/messages/', { params: { type_canal: 'direction' } }).then(res => setMessages(res.data.results || [])).finally(() => setLoading(false))
  }, [])

  const send = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!newMsg.trim()) return
    const res = await api.post('/messages/', { contenu: newMsg, type_canal: 'direction', type_message: 'texte' })
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-140px)] bg-white rounded-lg border border-neutral-200">
      <div className="p-4 border-b"><h2 className="text-lg font-semibold">👑 Messagerie Direction</h2></div>
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        ...
        {messages.length === 0 && !loading && <p className="text-center text-neutral-500 py-4">Aucune communication avec la direction.</p>}
      </div>
      <form onSubmit={send} className="p-3 border-t flex gap-2">
        <input value={newMsg} onChange={e => setNewMsg(e.target.value)} placeholder="Écrire à la direction..." className="form-input flex-1" />
