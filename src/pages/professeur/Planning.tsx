...
import { useGetPlanningQuery, useCreateSeanceDispoMutation, useUpdatePlanningItemMutation, useGetClassesQuery, usePauseClassMutation, useFlagDeleteClassMutation, useReactivateClassMutation } from '../../store/apiSlice';
import { useAppSelector } from '../../store/hooks';
import { selectAuth } from '../../store/authSlice';
import { PlanningFilters, PlanningItem } from '../../types';
import { Calendar, List, RefreshCw, CheckCircle, XCircle, Clock, AlertTriangle, BookOpen, PauseCircle, Trash2, ChevronRight, X } from 'lucide-react';
...
export default function PlanningPro({ professorId, hideHeader }: PlanningProProps) {
  ...
  const [updateSeance, { isLoading: isUpdating }] = useUpdatePlanningItemMutation();
  ...
  const doAction = async (classId: string, action: string, fn: () => Promise<any>) => {
    ...
    try { await fn(); refetchClasses(); refetch(); }
  ...
  const actionsFor = (classe: any): { label: string; icon: any; handler: () => void; style: string; key: string }[] => {
    ...
    if (s === 'active') return [
      ...
      { key: 'delete', label: 'Supprimer', icon: Trash2, handler: () => handleFlagDelete(id), style: 'bg-red-50 hover:bg-red-100 text-red-700 border-red-200' },
    ];
    if (s === 'pause') return [
  ...
  // Toujours filtrer par le prof connecté — jamais voir les séances des autres profs
  const queryFilters = useMemo(() => ({
    ...filters,
    ...(effectiveProfesseurId ? { professeur_id: effectiveProfesseurId } : {}),
  }), [filters, effectiveProfesseurId]);
...
function StatsBar({ data }: { data: PlanningItem[] }) {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
      {[
        { label: 'Sens des couleurs :', value: stats.total, color: 'bg-gray-50 text-gray-700' },
        { label: 'Disponibilités', value: stats.completed, color: 'bg-green-50 text-green-700' },
        { label: 'Créneaux des classes', value: stats.in_progress, color: 'bg-blue-50 text-blue-700' },
        { label: 'Classes mis en pause', value: stats.late, color: 'bg-orange-50 text-orange-700' },
        { label: 'Classes a suppimer', value: stats.absent, color: 'bg-red-50 text-red-700' },
